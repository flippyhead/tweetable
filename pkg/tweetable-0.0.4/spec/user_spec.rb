...
attrs = {:screen_name => 'flippyhead', :user_id => '13705052', :created_at => Time.now.to_s, :updated_at => Time.now.to_s}

describe Tweetable::User, 'when doing CRUD' do 
  before do 
    RedisSpecHelper.reset
  end
  
  it "should store a user item with screen_name, user_id, and created_at attributes" do
    user = Tweetable::User.create(:created_at => DateTime.now, :screen_name => 'username', :user_id => 12345)
    user.should be_valid
  end
  
  # it "should not allow a user without a name and id" do
  #   user = Tweetable::User.create(:created_at => DateTime.now)
  #   user.errors.should == [[:screen_name, :not_present]]
  # end
  ...
  it "should save user info after modification" do 
    ...
    user.user_id = 12345
    user.save    
    users = Tweetable::User.find(:screen_name, attrs[:screen_name])
    users.first.user_id.should == '12345'
  end

  it "should set created_at" do 
    Tweetable::User.create(attrs)
    user = Tweetable::User.find(:screen_name, attrs[:screen_name]).first
    user.created_at.should == attrs[:created_at]
  end
  
  it "should not save user info without calling save" do 
    user = Tweetable::User.create(attrs)
    user.user_id = '12345'
    users = Tweetable::User.find(:screen_name, attrs[:screen_name]).first
    users.user_id.should == attrs[:user_id]
  end
...
describe Tweetable::User, 'when updating info' do 
  before do 
    RedisSpecHelper.reset
    @user = Tweetable::User.create(attrs)
  end
  ...
  it "should get user_id" do
    @user.update_info
    users = Tweetable::User.find(:screen_name, attrs[:screen_name])
    users.first.user_id.should == attrs[:user_id]
  end  
...
describe Tweetable::User, 'when updating friends and followers' do 
  before do 
    RedisSpecHelper.reset
    @user = Tweetable::User.create(attrs)
  end
...
describe Tweetable::User, 'when updating messages' do 
  before do 
    RedisSpecHelper.reset
    @user = Tweetable::User.create(attrs)
  end
